...
type Cut = { id: number; type: "H" | "V"; x1: number; y1: number; x2: number; y2: number; boardIndex: number };
...
function packColumnShelves(
  ...
  while (sortable.length > 0) {
    ...
    for (let i = 0; i < sortable.length; ) {
      if (sortable[i].h <= stripHeight) { stripItems.push(sortable.splice(i, 1)[0]); } else { i++; }
    }

    // Remplir la bande de gauche à droite
    let x = colX; let used = 0; const rowPieces: PlacedPiece[] = [];
    ...
    const strip: Strip = { x: colX, width: colW, y, height: stripHeight, pieces: rowPieces, usedWidth: used };
    strips.push(strip);
...
function tryOneBoardTwoColumns(
  ...
  let counter = 1;
  for (const s of specs) for (let q = 0; q < s.qty; q++) items.push({ specId: s.id, w: s.w, h: s.h, id: `#${counter++}` });
  ...
  items.forEach(it => { candSet.add(it.w); candSet.add(it.h); });
  const candidates = Array.from(candSet).filter(w => w > 0 && w < boardW - 50).sort((a, b) => b - a);
  ...
  for (const splitX of candidates) {
    ...
    for (const it of itemsSorted) {
      ...
      if (fitL && !fitR) { left.push(it); const s = simulate(simL, colL.w, fitL); simL.totalH=s.totalH; simL.rowH=s.rowH; simL.rowRemW=s.rowRemW; }
      else if (!fitL && fitR) { right.push(it); const s = simulate(simR, colR.w, fitR); simR.totalH=s.totalH; simR.rowH=s.rowH; simR.rowRemW=s.rowRemW; }
      else if (fitL && fitR) {
        const sL = simulate({ ...simL }, colL.w, fitL);
        const sR = simulate({ ...simR }, colR.w, fitR);
        if (sL.predicted <= sR.predicted) { left.push(it); simL.totalH=sL.totalH; simL.rowH=sL.rowH; simL.rowRemW=sL.rowRemW; }
        else { right.push(it); simR.totalH=sR.totalH; simR.rowH=sR.rowH; simR.rowRemW=sR.rowRemW; }
    ...
    const board: BoardLayout = { index: 0, strips: [], width: boardW, height: boardH, columnSplits: [splitX] };
...
function packGuillotine(
  ...
  let counter = 1;
  for (const s of specs) for (let i = 0; i < s.qty; i++) expanded.push({ specId: s.id, w: s.w, h: s.h, id: `#${counter++}` });
  ...
  function newBoard(): BoardLayout {
    const b: BoardLayout = { index: boards.length, strips: [], width: boardW, height: boardH, columnSplits: [] };
    boards.push(b);
    return b;
  }
  ...
  for (const item of expanded) {
    ...
    for (const { st, idx } of candidateStrips) {
      ...
      const orients = allowRotate ? [ { w: item.w, h: item.h, rot: false }, { w: item.h, h: item.w, rot: true } ] : [ { w: item.w, h: item.h, rot: false } ];
      // préférer orientation qui colle au bord droit
      orients.sort((o1, o2) => Math.abs((st.x + st.width) - (nextX + o1.w)) - Math.abs((st.x + st.width) - (nextX + o2.w)));
      for (const o of orients) {
        if (o.h <= st.height && nextX + o.w <= st.x + st.width) {
          const piece: PlacedPiece = { id: item.id, specId: item.specId, w: o.w, h: o.h, rotated: o.rot, x: nextX, y: st.y, boardIndex: board.index, stripIndex: idx };
    ...
    if (stripY + chosen.h > boardH) { board = newBoard(); }

    const stripY2 = board.strips.reduce((acc, s, i) => acc + s.height + (i > 0 ? kerf : 0), 0) + (board.strips.length > 0 ? kerf : 0);
    const strip: Strip = { x: 0, width: boardW, y: stripY2, height: chosen.h, pieces: [], usedWidth: 0 };
    board.strips.push(strip);

    const piece: PlacedPiece = { id: item.id, specId: item.specId, w: chosen.w, h: chosen.h, rotated: chosen.rot, x: 0, y: strip.y, boardIndex: board.index, stripIndex: board.strips.length - 1 };
    strip.pieces.push(piece); strip.usedWidth = chosen.w; placed.push(piece);
  }
...
function computeCuts(boards: BoardLayout[], boardW: number, boardH: number, kerf: number) {
  ...
  for (const b of boards) {
    ...
    (b.columnSplits || []).forEach(x => {
      ...
      if (!keySet.has(k)) { keySet.add(k); cuts.push({ id: cid++, type: "V", x1: x, y1: 0, x2: x, y2: boardH, boardIndex: b.index }); }
    });
    ...
    for (let i = 0; i < b.strips.length; i++) {
      ...
      if (y < boardH && !keySet.has(k)) { keySet.add(k); cuts.push({ id: cid++, type: "H", x1: st.x, y1: y, x2: st.x + st.width, y2: y, boardIndex: b.index }); }
    }
    ...
    for (const st of b.strips) {
      for (let i = 0; i < st.pieces.length - 1; i++) {
        ...
        if (!keySet.has(k)) { keySet.add(k); cuts.push({ id: cid++, type: "V", x1: x, y1: st.y, x2: x, y2: st.y + st.height, boardIndex: b.index }); }
      }
      ...
      if (last) {
        ...
        if (xRight < st.x + st.width - 1e-6) {
          ...
          if (!keySet.has(k2)) { keySet.add(k2); cuts.push({ id: cid++, type: "V", x1: xRight, y1: st.y, x2: xRight, y2: st.y + st.height, boardIndex: b.index }); }
        }
      }
    }
  }
...
function BoardSVG({ board, boardW, boardH, unit, cuts, zoom }: { board: BoardLayout; boardW: number; boardH: number; unit: Unit; cuts: Cut[]; zoom: number }) {
  const baseWpx = 1100; // base display width (agrandi)
  const baseHpx = 700;  // base display height (agrandi)
  const scale = Math.min((baseWpx * Math.max(0.5, zoom)) / boardW, (baseHpx * Math.max(0.5, zoom)) / boardH);
  ...
  return (
    <div className="rounded-2xl border p-4 shadow-sm bg-white">
      <div className="flex items-baseline justify-between mb-2">
        <h3 className="font-semibold text-lg">🪵 Planche #{board.index + 1} <span className="text-gray-500">({fmt(boardW, unit)} × {fmt(boardH, unit)})</span></h3>
        <span className="text-sm text-gray-600">{board.strips.reduce((n, s) => n + s.pieces.length, 0)} pièce(s) • {boardCuts.length} coupe(s)</span>
      </div>
      <div className="overflow-visible">
        <svg width={wpx} height={hpx} className="bg-gray-50 border rounded" style={{ imageRendering: "crisp-edges" }}>
          {/* outer border */}
          <rect x={0} y={0} width={wpx} height={hpx} fill="#f8fafc" stroke="#1f2937" strokeWidth={1} />

          {/* column split guides */}
          {(board.columnSplits || []).map((x, i) => (
            <line key={`split-${i}`} x1={x * scale} y1={0} x2={x * scale} y2={hpx} stroke="#10b981" strokeDasharray="4 6" strokeWidth={1.5} />
          ))}
          ...
          {board.strips.map((st, si) => (
            <g key={si}>
              {st.pieces.map((p) => {
                ...
                return (
                  <g key={p.id}>
                    <rect x={x} y={y} width={w} height={h} fill="#bfdbfe" stroke="#2563eb" strokeWidth={1} />
                    <text x={x + 6} y={y + 16} fontSize={fsMain} fill="#1f2937">{p.id}</text>
                    <text x={x + 6} y={y + 32} fontSize={fsDims} fill="#374151">{`${Math.round(p.w)}×${Math.round(p.h)} mm${p.rotated ? " ↻" : ""}`}</text>
                  </g>
                );
              })}
            </g>
          ...
          {/* cuts overlay */}
          {boardCuts.map(c => {
            const x1 = c.x1 * scale; const y1 = c.y1 * scale; const x2 = c.x2 * scale; const y2 = c.y2 * scale;
            return (
              <g key={`c${c.id}`}>
                <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#ef4444" strokeDasharray="6 6" strokeWidth={cutStroke} />
                <circle cx={(x1 + x2) / 2} cy={(y1 + y2) / 2} r={cutRadius} fill="#ef4444" />
                <text x={(x1 + x2) / 2} y={(y1 + y2) / 2 + 4} textAnchor="middle" fontSize={fsMain} fill="#fff">{c.id}</text>
              </g>
            );
          })}
        </svg>
...
function PositioningReport({ boards, cuts, unit }: { boards: BoardLayout[]; cuts: Cut[]; unit: Unit }) {
  return (
    <div className="rounded-2xl border p-4 shadow-sm bg-white">
      <h3 className="font-semibold text-lg mb-2">📝 Compte‑rendu de positionnement</h3>
      <div className="space-y-3 text-sm">
        {boards.map((b) => {
          ...
          return (
            <div key={b.index}>
              <div className="font-medium mb-1">Planche #{b.index + 1} — {fmt(b.width, unit)} × {fmt(b.height, unit)}</div>
              {[...byColumn.entries()].sort((a,b) => parseFloat(a[0].split('-')[0]) - parseFloat(b[0].split('-')[0])).map(([key, strips], idx) => {
                const [sx, sw] = key.split('-').map(Number);
                strips.sort((a, b2) => a.y - b2.y);
                return (
                  <div key={key} className="ml-2">
                    <div className="text-gray-700">Colonne {idx + 1} : X {fmt(sx, unit)} → {fmt(sx + sw, unit)} (largeur {fmt(sw, unit)})</div>
                    {strips.map((st, si) => (
                      <div key={si} className="ml-4">
                        <div>• Bande {si + 1} @ Y={fmt(st.y, unit)} hauteur {fmt(st.height, unit)} :</div>
                        <div className="ml-4">
                          {st.pieces.map(p => (
                            <div key={p.id}>– {p.id} ({p.specId}) {p.rotated ? "↻ " : ""}{Math.round(p.w)}×{Math.round(p.h)} mm @ (x={Math.round(p.x)}, y={Math.round(p.y)})</div>
                          ))}
                        </div>
                      </div>
                    ))}
              ...
              <div className="mt-1 text-gray-700">Coupes sur la planche : {cuts.filter(c => c.boardIndex === b.index).length}</div>
            </div>
          );
        })}
      </div>
...
function VerificationCard({ boards, allPieces, specs, boardW, boardH, unit }: { boards: BoardLayout[]; allPieces: PlacedPiece[]; specs: PieceSpec[]; boardW: number; boardH: number; unit: Unit }) {
  ...
  allPieces.forEach(p => { perSpecPlaced[p.specId] = (perSpecPlaced[p.specId] || 0) + 1; });
  ...
  return (
    <div className="rounded-2xl border p-4 shadow-sm bg-white">
      <h3 className="font-semibold text-lg mb-2">✅ Carte de vérification</h3>
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <div>Pièces placées : <b>{allPieces.length}</b></div>
          <div>Surface pièces : <b>{fmt(areaPieces, unit)}²</b></div>
          <div>Surface planches ({boards.length}) : <b>{fmt(boardsArea, unit)}²</b></div>
          <div>Taux d'utilisation : <b>{(utilization * 100).toFixed(1)}%</b></div>
        </div>
        <div>
          <div className="font-medium mb-1">Quantités par référence</div>
          {specs.map(s => (
            <div key={s.id} className="flex justify-between">
              <span>{s.id} — {s.w}×{s.h} mm</span>
              <span>{perSpecPlaced[s.id] || 0} / {s.qty} {(perSpecPlaced[s.id] === s.qty) ? "✓" : "✗"}</span>
            </div>
          ))}
          <div className="mt-2">Statut : {allMatch ? <span className="text-green-600 font-semibold">OK – toutes les pièces sont présentes</span> : <span className="text-red-600 font-semibold">Erreur – pièces manquantes</span>}</div>
        </div>
      </div>
    </div>
  );
...
function TestsCard({ boards, cuts, specs, allPieces, boardW, boardH, kerf, unit }: { boards: BoardLayout[]; cuts: Cut[]; specs: PieceSpec[]; allPieces: PlacedPiece[]; boardW: number; boardH: number; kerf: number; unit: Unit }) {
  const eps = 1e-6;
  const issues: string[] = [];
  ...
  if (expectedArea === actualArea) passes.push("Surface totale des découpes = somme attendue des références ✓"); else issues.push(`Surface mismatch: attendu ${expectedArea} mm², calculé ${actualArea} mm²`);

  // Helper: per-spec counts
  const perSpecExp: Record<string, number> = {}; specs.forEach(s => perSpecExp[s.id] = s.qty);
  const perSpecGot: Record<string, number> = {}; allPieces.forEach(p => perSpecGot[p.specId] = (perSpecGot[p.specId]||0)+1);
  for (const s of specs) {
    if ((perSpecGot[s.id]||0) !== s.qty) issues.push(`Quantité ${s.id}: ${perSpecGot[s.id]||0}/${s.qty}`);
  }
  if (!issues.find(t => t.startsWith("Quantité "))) passes.push("Quantités par référence conformes ✓");
  ...
  boards.forEach(b => {
    ...
    pieces.forEach(p => {
      if (p.x < -eps || p.y < -eps || p.x + p.w > boardW + eps || p.y + p.h > boardH + eps) issues.push(`${p.id}: en dehors des limites planche`);
      ...
      if (!st) warnings.push(`${p.id}: bande non trouvée (tolérance) – vérif visuelle recommandée`);
        ...
        if (p.x + p.w > st.x + st.width + eps || p.x < st.x - eps) issues.push(`${p.id}: dépasse la colonne`);
        if (p.h - st.height > eps) warnings.push(`${p.id}: plus haut que la bande (${p.h} > ${st.height})`);
      }
    });
  });
  if (!issues.some(t => t.includes("limites") || t.includes("colonne"))) passes.push("Aucune pièce hors limites/colonne ✓");
  ...
  boards.forEach(b => {
    ...
    for (let i=0;i<pieces.length;i++) for (let j=i+1;j<pieces.length;j++) {
      if (overlap(pieces[i], pieces[j])) issues.push(`Chevauchement: ${pieces[i].id} & ${pieces[j].id} (planche ${b.index+1})`);
    }
  });
  if (!issues.find(t => t.startsWith("Chevauchement"))) passes.push("Aucun chevauchement détecté ✓");

  // Kerf checks: horizontal dans chaque bande
  boards.forEach(b => {
    const byCol = new Map<string, Strip[]>();
    b.strips.forEach(st => { const key = `${st.x}-${st.width}`; if (!byCol.has(key)) byCol.set(key, []); byCol.get(key)!.push(st); });
    for (const [, sts] of byCol) {
      ...
      for (let i=1;i<sts.length;i++) {
        ...
        if (diff > 1e-6) warnings.push(`Ecart kerf vertical entre bandes @X=${sts[i].x}: ${diff.toFixed(3)} mm`);
      }
      ...
      for (const st of sts) {
        ...
        for (let i=1;i<sorted.length;i++) {
          ...
          if (diff > 1e-6) warnings.push(`Ecart kerf horizontal dans une bande @Y=${st.y}: ${diff.toFixed(3)} mm`);
        }
      }
    }
  });
  ...
  if (duplicates.length>0) issues.push(`Coupes dupliquées: ${duplicates.length}`); else passes.push("Pas de double‑comptage des coupes ✓");

  return (
    <div className="rounded-2xl border p-4 shadow-sm bg-white">
      <h3 className="font-semibold text-lg mb-2">🧪 Tests automatiques</h3>
      <div className="grid md:grid-cols-3 gap-4 text-sm">
        <div>
          <div className="font-medium mb-1">Pass ✅</div>
          {passes.length===0 ? <div className="text-gray-500">—</div> : passes.map((t,i)=>(<div key={i}>• {t}</div>))}
        </div>
        <div>
          <div className="font-medium mb-1">Avertissements ⚠️</div>
          {warnings.length===0 ? <div className="text-gray-500">—</div> : warnings.map((t,i)=>(<div key={i}>• {t}</div>))}
        </div>
        <div>
          <div className="font-medium mb-1">Erreurs ❌</div>
          {issues.length===0 ? <div className="text-gray-500">—</div> : issues.map((t,i)=>(<div key={i}>• {t}</div>))}
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // Cuts
  const cuts = useMemo(() => computeCuts(boards, boardW, boardH, kerf), [boards, boardW, boardH, kerf]);
  ...
  // Actions
  function addSpec() {
    setSpecs(prev => [...prev, { id: String.fromCharCode(65 + prev.length), w: 100, h: 100, qty: 1 }]);
  }
  function updateSpec(idx: number, patch: Partial<PieceSpec>) { setSpecs(prev => prev.map((s, i) => i === idx ? { ...s, ...patch } : s)); }
  function removeSpec(idx: number) { setSpecs(prev => prev.filter((_, i) => i !== idx)); }
  function resetExample() {
    setUnit("mm"); setBoardWInput("1500"); setBoardHInput("5000"); setKerfInput("3"); setAllowRotate(true); setObjective("balanced"); setForceTwoCols(true);
    setSpecs([
      { id: "A", w: 930, h: 750, qty: 5 },
      { id: "B", w: 300, h: 800, qty: 3 },
      { id: "C", w: 450, h: 600, qty: 4 },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-gray-600">Deux stratégies : (1) essai prioritaire sur <b>1 planche en 2‑colonnes</b> (two‑stage guillotine, bandes horizontales par colonne), (2) fallback en <b>bandes pleine largeur</b>. Compteur de coupes exact et visualisation proportionnelle.</p>
      ...
      <div className="rounded-2xl border p-4 shadow-sm bg-white">
        <div className="grid md:grid-cols-5 gap-4">
          <div>
            ...
            <select value={unit} onChange={e => setUnit(e.target.value as Unit)} className="mt-1 w-full border rounded px-2 py-1">
          ...
          <div>
            ...
            <input value={boardWInput} onChange={e => setBoardWInput(e.target.value)} className="mt-1 w-full border rounded px-2 py-1" />
          </div>
          <div>
            <label className="block text-sm font-medium">Longueur planche</label>
            <input value={boardHInput} onChange={e => setBoardHInput(e.target.value)} className="mt-1 w-full border rounded px-2 py-1" />
          </div>
          <div>
            <label className="block text-sm font-medium">Trait de scie (kerf)</label>
            <input value={kerfInput} onChange={e => setKerfInput(e.target.value)} className="mt-1 w-full border rounded px-2 py-1" />
          </div>
          <div>
            <label className="block text-sm font-medium">Essai 1 planche (2‑colonnes)</label>
            <div className="mt-1 flex items-center gap-2"><input id="twoCols" type="checkbox" checked={forceTwoCols} onChange={e => setForceTwoCols(e.target.checked)} /><label htmlFor="twoCols">Activer</label></div>
          </div>
        </div>
        <div className="mt-4 grid md:grid-cols-4 gap-4 items-center">
          <div className="flex items-center gap-2">
            <input id="rot" type="checkbox" checked={allowRotate} onChange={e => setAllowRotate(e.target.checked)} />
            <label htmlFor="rot">Autoriser la rotation 90°</label>
          </div>
          <div>
            <label className="block text-sm font-medium">Objectif (fallback)</label>
            <select value={objective} onChange={e => setObjective(e.target.value as any)} className="mt-1 w-full border rounded px-2 py-1">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              <button className="px-2 py-1 border rounded" onClick={() => setZoom(z => Math.max(0.5, +(z - 0.2).toFixed(1)))}>−</button>
              <input type="range" min={0.6} max={3} step={0.1} value={zoom} onChange={e => setZoom(parseFloat(e.target.value))} className="w-40" />
              <button className="px-2 py-1 border rounded" onClick={() => setZoom(z => Math.min(3, +(z + 0.2).toFixed(1)))}>＋</button>
              <span className="text-sm text-gray-600 w-12 text-right">x{zoom.toFixed(1)}</span>
              <button className="px-2 py-1 border rounded bg-gray-100" onClick={() => setZoom(1)}>100%</button>
              <button className="px-2 py-1 border rounded bg-blue-50 text-blue-700" onClick={() => setZoom(2)}>Agrandir</button>
            </div>
          </div>
          <div className="flex gap-2 justify-end">
            <button className="px-3 py-2 rounded bg-gray-100" onClick={resetExample}>🔄 Réinitialiser l'exemple</button>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border p-4 shadow-sm bg-white">
        <div className="flex justify-between items-center mb-2">
          ...
          <button className="px-3 py-2 rounded bg-blue-600 text-white" onClick={addSpec}>➕ Ajouter</button>
        </div>
        ...
        {specs.map((s, idx) => (
          <div key={idx} className="grid grid-cols-12 gap-2 items-center mb-1">
            <input className="col-span-2 border rounded px-2 py-1" value={s.id} onChange={e => updateSpec(idx, { id: e.target.value })} />
            <input className="col-span-3 border rounded px-2 py-1" type="number" value={s.w} onChange={e => updateSpec(idx, { w: Number(e.target.value) })} />
            <input className="col-span-3 border rounded px-2 py-1" type="number" value={s.h} onChange={e => updateSpec(idx, { h: Number(e.target.value) })} />
            <input className="col-span-2 border rounded px-2 py-1" type="number" value={s.qty} onChange={e => updateSpec(idx, { qty: Number(e.target.value) })} />
            <button className="col-span-2 px-3 py-2 rounded bg-red-50 text-red-700" onClick={() => removeSpec(idx)}>Supprimer</button>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border p-4 shadow-sm bg-white">
        <div className="flex flex-wrap gap-4 items-center">
          <div className="text-sm">Planches utilisées : <b>{boards.length}</b></div>
          <div className="text-sm">Pièces placées : <b>{allPieces.length}</b></div>
          <div className="text-sm">Coupes totales : <b>{computeCuts(boards, boardW, boardH, kerf).length}</b></div>
          <div className="text-sm">Utilisation : <b>{(usage * 100).toFixed(1)}%</b></div>
        </div>
      </div>
      ...
      <div className={`grid ${zoom >= 1.6 ? 'grid-cols-1' : 'md:grid-cols-2'} gap-4`}>
        {boards.map(b => (
          <BoardSVG key={b.index} board={b} boardW={boardW} boardH={boardH} unit={unit} cuts={cuts} zoom={zoom} />
        ))}
      </div>
      ...
      {/* Verification */}
      <VerificationCard boards={boards} allPieces={allPieces} specs={specs} boardW={boardW} boardH={boardH} unit={unit} />

      <div className="text-xs text-gray-500">
        Astuce : l’essai « 2‑colonnes » correspond aux <em>two‑stage guillotine patterns</em> : d’abord scission verticale maîtresse, puis bandes horizontales par colonne. Placez la première bande à y=0 pour économiser une coupe.
      </div>
    </div>
  );
}
